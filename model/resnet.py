...
def remove_layer(state_dict,layer):
    """ Remove the fc layer parameter from state_dict. """
    return {key: value for key, value in state_dict.items() if not key.startswith(layer)}
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        # state_dict=remove_layer(state_dict,layer='layer4.2') #### resnet50 cat layer
        #state_dict=remove_layer(state_dict,layer='layer3.5')
...
def resnet50(pretrained=True, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 2], pretrained, progress,
                   **kwargs)
